...
| <PyObject object="AssetsDefinition" /> | A class that represents a one or more asset definitions, usually backed by a single function that materializes or observes them all together. |
...
Assets can depend on assets in different [code locations](/concepts/code-locations). Consider this example for `code_location_1`:

```python file=/concepts/assets/cross_cl_code_location_1.py startafter=start endbefore=end
# code_location_1.py
import json
...
```python file=/concepts/assets/cross_cl_code_location_2.py startafter=start endbefore=end
# code_location_2.py
import json
...
Check out the [Metadata & tags](/concepts/metadata-tags) documentation to get started.
